...
export function ProductBadges({
  ...
  const badgeClass = cn(
    'm-1 bg-background text-foreground hover:bg-background rounded-(--badges-corner-radius)',
  );
    ...
    >
      ...
      {isSoldOut && (
        <Badge
          ...
          variant="secondary"
        >
          {themeContent?.product?.soldOutProductBadge || themeContent?.product?.soldOut}
        </Badge>
      )}
    </div>
  );
